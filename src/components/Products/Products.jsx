...
function Products() {
    ...
    useEffect(() => {
        ...
        fetch("https://libreriahedwig.herokuapp.com/products/api/v1/autores")
        .then(res => res.json())
        .then(res => { setAutores({ autores: res }) })

        fetch("https://libreriahedwig.herokuapp.com/products/api/v1/generos")
        .then(res => res.json())
        ...
        <tbody>
            {
                productos.productos.products !== undefined ? productos.productos.products.map((libro, i) => {
                    ...
                    let link = "https://libreriahedwig.herokuapp.com/products/detalles/" + libro.id
                    return (
                        <tr key={i}>
